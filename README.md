...
- **Bulk Import** `/import` for operators: paste rows from Excel, Google Sheets, or a CSV (or upload a CSV), check them in a spreadsheet grid, and record every ready row as an ordinary manual gift. Columns are matched by header name when a header row is included; the server's guards (large-gift confirmation, possible duplicate, card collision) apply per row and are resolved in the sheet's Status and Confirm columns. The sheet is kept in the browser until it is cleared, and each row carries a fixed id so re-running after a lost connection never records a gift twice.
...
| Internet is down for the venue | Run Givebar on a laptop on the room's own Wi-Fi (`bun run server/src/index.ts`, share `http://<laptop-ip>:3000`). Sign-in works over plain HTTP on a private network; online gifts and invites need the internet back. |
