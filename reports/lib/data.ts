...
export type Takeaway = { kind: "win" | "action" | "watch" | "insight"; title: string; body: string; href?: string; link?: string };
...
export function buildTakeaways(r: Report): Takeaway[] {
  ...
  if (s.declined_count) out.push({ kind: "action", href: "followup.html#declined", link: "Declined payments", title: `${s.declined_count} online payments were declined (${money(s.declined_cents)})`, body: `These people tried to give and their card failed. Email them the donate link with a short note that the payment did not go through. The list is on the Follow-up page.` });
  if (s.prospects_total) out.push({ kind: "insight", href: "crossref.html?prospects=below", link: "Ask list, below ask", title: `${s.prospects_gave} of ${s.prospects_total} people on the ask list gave; ${s.prospects_under_ask.length} gave less than their ask`, body: `Asks on the list total ${money(s.prospects_ask_cents)}; the people who gave recorded ${money(s.prospects_actual_cents)}. Call the under-ask group this month; they already said yes to something.` });
  if (s.anonymous_count) out.push({ kind: "insight", href: "donors.html?f=anon", link: "Anonymous donors", title: `${s.anonymous_count} anonymous gifts, ${money(s.anonymous_cents)}`, body: `${pct(s.anonymous_cents / s.total_cents)} of the total. Their names are in this report and the workbook for the team only. Thank them privately; never print them.` });
  ...
  if (s.recurring_count) out.push({ kind: "win", href: "gifts.html?q=monthly", link: "Monthly gifts", title: `${s.recurring_count} new monthly donors`, body: `${money(s.recurring_monthly_cents)} per month started tonight, ${money(s.recurring_monthly_cents * 12)} a year. Send them a welcome note this week.` });
  ...
  if (s.ticket_buyers) out.push({ kind: "action", href: "followup.html#tickets", link: "Ticket buyers without a gift", title: `${s.ticket_buyers - s.ticket_buyers_gave} of ${s.ticket_buyers} ticket buyers have no gift recorded`, body: `They were in the room and heard the appeal. Email them first.` });
  ...
  if (!r.bloomerang.connected) out.push({ kind: "action", title: "Connect Bloomerang to see repeat-donor history", body: r.bloomerang.message });
    ...
    out.push({ kind: "insight", href: "donors.html?f=new", link: "First-time donors", title: `${s.repeat_donors} repeat donors gave ${money(b.repeat_cents)}; ${s.new_donors} first-time donors gave ${money(b.new_cents)}`, body: `${b.matched} of ${s.households} households matched a Bloomerang record; ${s.unknown_donors} did not. Send first-time donors a welcome email within 7 days. Send repeat donors a thank-you that mentions their last gift.` });
    if (b.returning.count) out.push({ kind: b.returning.now_cents >= b.returning.then_cents ? "win" : "watch", href: "index.html#history", link: "Returning donors", title: `${b.returning.count} donors from last year's gala gave again: ${money(b.returning.then_cents)} last year, ${money(b.returning.now_cents)} this year`, body: `${b.returning.upgraded} gave more than last year, ${b.returning.downgraded} gave less. Call the people who gave less first. A board member should thank the people who gave more.` });
    if (b.lapsed.length) out.push({ kind: "action", href: "followup.html#lapsed", link: "Lapsed gala donors", title: `${b.lapsed.length} donors gave at last year's gala (${money(b.lapsed.reduce((n, l) => n + l.last_gala_cents, 0))}) and have no gift recorded this year`, body: `Some of them gave under a spouse's or business name; check the Follow-up list before calling. Then email them as a separate group.` });
  }
